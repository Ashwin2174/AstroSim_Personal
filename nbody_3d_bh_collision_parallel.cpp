...
class solver: public BarnesHut {
    ...
    public:
        ...
        void leapfrog(vector<Body>& o, long double h){
            ...
            vector<long double> pzd3 = forces[2];
            for (int i=0;i<n;i++){
                o[i].momentum[0] += 0.5*h*pxd3[i]; //px4
                o[i].momentum[1] += 0.5*h*pyd3[i]; //py4
                o[i].momentum[2] += 0.5*h*pzd3[i]; //pz4
                o[i].position[0] += h*o[i].momentum[0]/o[i].mass; 
                o[i].position[1] += h*o[i].momentum[1]/o[i].mass; 
                o[i].position[2] += h*o[i].momentum[2]/o[i].mass;
            }
            forces = BarnesHutForces(o);
            pxd3 = forces[0];
            pyd3 = forces[1];
            pzd3 = forces[2];
            for (int i=0;i<n;i++){
...
int main(){
    ...
    long double t_end = 40000;
    long double h = 10;

    solver gravity_solver = solver({0,0,0});
    ifstream input("infile2.txt");
    vector<Body> o;
    for (int i =0;i<25000;i++){
        long double m,x,y,z,vx,vy,vz,id,r;
        input>>id>>m>>r>>x>>y>>z>>vx>>vy>>vz; 
        if (i%1==0){
        o.push_back(Body(m,50,x,y,z,vx,vy,vz));
        }
    }
    o[0].radius = 1000;
    o.push_back(Body(2000,250,60000,0,40000,-5,0,-50));
    cout<<o.size()<<endl;
    cout<<"-------------------------------------------------------------------"<<endl;
