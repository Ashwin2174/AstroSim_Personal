...
class Body {
    public:
        ...
        double radius;
        Body(double m,double r,double x,double y,double px,double py){
            mass = m;
            radius = r;
            position = {x,y};
            momentum = {px,py};
...
class solver {
    private:
        vector<Body> collision(vector<Body> o, int n, double e){
            for (int i=0;i<n-1;i++){
                for (int j=i+1;j<n;j++){
                    double xr = o[j].position[0] - o[i].position[0];
                    double yr = o[j].position[1] - o[i].position[1];
                    double d = sqrt(xr*xr+yr*yr);
                    double k = o[i].radius+o[j].radius-d;
                    if (d <= (o[i].radius+o[j].radius)){
                        double delpx = (1+e)*o[i].mass*o[j].mass*(xr*(o[j].momentum[0]/o[j].mass - o[i].momentum[0]/o[i].mass)+yr*(o[j].momentum[1]/o[j].mass - o[i].momentum[1]/o[i].mass))*(xr)/((o[i].mass+o[j].mass)*(d*d));
                        double delpy = (1+e)*o[i].mass*o[j].mass*(xr*(o[j].momentum[0]/o[j].mass - o[i].momentum[0]/o[i].mass)+yr*(o[j].momentum[1]/o[j].mass - o[i].momentum[1]/o[i].mass))*(yr)/((o[i].mass+o[j].mass)*(d*d));
    ...
    public:
        vector<Body> rk4(vector<Body> o, int n, double h, double e){
            vector<Body> k1_body = k1(o,n,h);
            vector<Body> k2_body = k2(o,n,h,k1_body);
            vector<Body> k3_body = k3(o,n,h,k2_body);
            vector<Body> k4_body = k4(o,n,h,k3_body);
            for (int i=0;i<n;i++){
...
int main(){
    vector<Body> o ={Body(10,1,0,0,0,0),Body(1,0.1,0,8,0.4,-0.4),Body(1,0.1,10,0,0.4,0.8)};
    double t_start = 0;
    double t_end = 100;
    double h = 0.001;
    ...
    for(double t=t_start;t<=t_end;t+=h){
        o = gravity_solver.rk4(o,o.size(),h,0.2);
        if (int(t/h)%100==0){
            file<<o[0].position[0]<<","<<o[0].position[1]<<","<<o[1].position[0]<<","<<o[1].position[1]<<","<<o[2].position[0]<<","<<o[2].position[1]<<'\n';
        }
    }
    file.close();
    cout<<"Completed"<<endl;
